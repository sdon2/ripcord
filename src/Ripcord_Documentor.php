	...
	function getIntrospectionXML()
	{
		...
		if (isset($this->methods) && is_array($this->methods)) {
			foreach ($this->methods as $method => $methodData) {
				...
				if (is_array($data)) {
					foreach ($data as $key => $value) {
						switch ($key) {
							...
							case 'deprecated':
							case 'package':
								$description .= '<div class="' . $key . '"><span class="tag">' . $key . '</span>' . $value . '</div>';
								break;
							default:
								break;
						}
				...
				$xml .= '<methodDescription name="' . $method . '"><purpose><![CDATA['. $description . ']]></purpose>';
				if (is_array($data) && (isset($data['arguments']) || isset($data['return']))) {
					...
					if (isset($data['arguments']) && is_array($data['arguments'])) {
						...
						foreach ($data['arguments'] as $name => $argument) {
							...
							$xml .= '<value type="' . htmlspecialchars($argument['type']) . '" name="' . htmlspecialchars($name) . '"><![CDATA[' . $argument['description'] . ']]></value>';
						}
						$xml .= '</params>';
					}
					if (isset($data['return']) && is_array($data['return'])) {
						$xml .= '<returns><value type="' . htmlspecialchars($data['return']['type']) . '"><![CDATA[' . $data['return']['description'] . ']]></value></returns>';
					}
